...
本文档承接 `docs/REQUIREMENTS.md` 的实现设计，是设计层面的唯一权威基线。
...
| `AssetTypeStore` | `register_asset_type`, `register_format`, `list_asset_types`, `get_asset_type`, `get_format`, `list_formats`. |
| `AssetStore` | `create_asset`, `get_asset`, `list_assets`, `update_asset`, `rename_asset`（支持同 Domain 内跨 Namespace 移动，见 §4.3）, `soft_delete_asset`, `restore_asset`, `hard_delete_asset`. |
...
| `TabularStore` | `create_tabular_asset`, `get_tabular_asset`：table 类型资产的扩展表读写通道（通用 `create_asset` 只写 `assets` 行，tabular 资产需同事务写 `tabular_assets` 扩展行，见 §3.3）。Iceberg 与 Lance adapter 共用。 |
...
>
> 与通用 `CasCommitStore` 一致，事务与视图的 commit 同样镜像版本：`IcebergTableCommit` 携带 `new_version_key`（由 adapter 从新 metadata_location 文件名提取序号），`commit_transaction_tables` 在单事务内逐表完成"锁→比对→插镜像版本→更新指针"（§6.4 S1–S6），任一失败整体回滚；`IcebergViewStore::commit_view` 同样携带 `new_version_key` 并镜像。Iceberg adapter 的 Router 状态类型为 `Arc<dyn IcebergCatalogStore>`。
...
pub trait AssetStore: Send + Sync {
    ...
    /// 改名，可选地在同一 Domain 内跨 Namespace 移动：new_namespace_path 为 None 时仅改名；
    /// Some(path) 时移动到资产所属 Domain 内该路径的 Namespace（不存在 → NotFound）；
    /// 目标 Namespace 内同名活动资产冲突 → AlreadyExists（由 uq_assets_active_name 兜底）。
    async fn rename_asset(&self, id: Uuid, new_name: &str, new_namespace_path: Option<&str>) -> Result<Asset, CatalogError>;
...
/// tabular 资产的类型扩展读写（见 §3.3 `tabular_assets`）。
///
...
pub trait TabularStore: Send + Sync {
    ...
    /// `input.asset_type` 必须为 `table`（应用层校验，不使用数据库触发器）。
...
pub trait CatalogStore: DomainStore + NamespaceStore + AssetTypeStore + AssetStore + VersionStore + TagStore + UnifiedQueryStore + CasCommitStore + TabularStore + Send + Sync {}
```
...
以下设计细节留待后续实现设计文档确定。本节关注**实现层面未明确的技术方案**；需求层面的功能与约束延期事项见 `docs/REQUIREMENTS.md` §10。
